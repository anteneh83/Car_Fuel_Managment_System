...
import { VehicleStatus, DriverStatus, RiskLevel, FuelRequestStatus } from '../types';
...
export class DashboardService {
  static async getSummary(query: { startDate?: string; endDate?: string }) {
    ...
    const [fuelStats, activeVehicles, activeDrivers, pendingRequests, highRisk, critical] =
      await Promise.all([
        FuelTransaction.aggregate([
          { $match: txFilter },
          {
            $group: {
              ...
              totalLiters: { $sum: '$fuelQuantity' },
              totalTx: { $sum: 1 },
              avgConsumption: { $avg: '$variancePercentage' },
            },
          },
        ]),
        Vehicle.countDocuments({ status: VehicleStatus.ACTIVE }),
        Driver.countDocuments({ status: DriverStatus.ACTIVE }),
        FuelRequest.countDocuments({ status: FuelRequestStatus.PENDING }),
  ...
  static async getFuelCost(query: { startDate?: string; endDate?: string }) {
    ...
    return FuelTransaction.aggregate([
      ...
      {
        $group: {
          _id: { year: { $year: '$fuelDate' }, month: { $month: '$fuelDate' } },
          totalCost: { $sum: '$totalAmount' },
          totalLiters: { $sum: '$fuelQuantity' },
        },
      },
  ...
  static async getVehicleUsage(query: { startDate?: string; endDate?: string }) {
    ...
    if (query.startDate || query.endDate) {
      filter.fuelDate = {};
      if (query.startDate) filter.fuelDate.$gte = new Date(query.startDate);
      if (query.endDate) filter.fuelDate.$lte = new Date(query.endDate);
    }
    ...
    return FuelTransaction.aggregate([
      ...
      { $unwind: '$vehicle' },
      {
        $group: {
          _id: '$vehicleId',
          vehicleName: { $first: '$vehicle.vehicleName' },
  ...
  static async getConsumptionTrend(query: { startDate?: string; endDate?: string }) {
    ...
    return FuelTransaction.aggregate([
      { $match: { fuelDate: { $gte: start, $lte: end } } },
      {
        $group: {
          _id: { year: { $year: '$fuelDate' }, month: { $month: '$fuelDate' } },
          expectedFuel: { $sum: '$expectedFuel' },
          claimedFuel: { $sum: '$fuelQuantity' },
        },
      },
      { $sort: { '_id.year': 1, '_id.month': 1 } },
    ]);
  ...
  static async getMonitorSummary(monitorId: string) {
    const now = new Date();
    const monthStart = startOfMonth(now);
    const monthEnd = endOfMonth(now);

    const [totalTransactions, pendingRequests, approvedRequests, monthlyStats] = await Promise.all([
      FuelTransaction.countDocuments({ monitorId }),
      FuelRequest.countDocuments({ monitorId, status: FuelRequestStatus.PENDING }),
      FuelRequest.countDocuments({ monitorId, status: FuelRequestStatus.APPROVED }),
      FuelTransaction.aggregate([
        { $match: { monitorId: monitorId as any, fuelDate: { $gte: monthStart, $lte: monthEnd } } },
        {
          $group: {
            _id: null,
            totalLiters: { $sum: '$fuelQuantity' },
