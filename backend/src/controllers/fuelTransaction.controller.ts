...
export class FuelTransactionController {
  ...
  static async create(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const tx = await FuelTransactionService.create({
        fuelRequestId: req.body.fuelRequestId,
      ...
      res.status(201).json({
        success: true,
        data: tx,
        message: 'Fuel transaction submitted successfully',
      });
    } catch (error) {
      next(error);
    }
  ...
  static async getMyTransactions(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const result = await FuelTransactionService.getMyTransactions(req.user!.userId, req.query);
      res.json({ success: true, ...result });
    } catch (error) {
      next(error);
    }
  ...
  static async review(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const tx = await FuelTransactionService.review(
      ...
      await AuditLogService.log({
        userId: req.user!.userId,
        userRole: req.user!.role,
        action: AuditAction.FUEL_TRANSACTION_REVIEWED,
        entityType: 'FuelTransaction',
        entityId: tx._id as any,
        newValue: { reviewStatus: 'REVIEWED', notes: req.body.notes },
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });
  ...
  static async investigate(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const tx = await FuelTransactionService.investigate(
        req.params.id as string,
        req.user!.userId,
        req.body.notes
      );
      await AuditLogService.log({
        userId: req.user!.userId,
        userRole: req.user!.role,
        action: AuditAction.FUEL_TRANSACTION_INVESTIGATED,
        entityType: 'FuelTransaction',
        entityId: tx._id as any,
        newValue: { reviewStatus: 'INVESTIGATED', notes: req.body.notes },
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });
  ...
  static async resolve(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const tx = await FuelTransactionService.resolve(
        req.params.id as string,
        req.user!.userId,
        req.body.notes
      );
      await AuditLogService.log({
        userId: req.user!.userId,
        userRole: req.user!.role,
        action: AuditAction.FUEL_TRANSACTION_RESOLVED,
        entityType: 'FuelTransaction',
        entityId: tx._id as any,
        newValue: { reviewStatus: 'RESOLVED', notes: req.body.notes },
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
      });
