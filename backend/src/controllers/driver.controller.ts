...
export class DriverController {
  ...
  static async create(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      res.status(201).json({
        success: true,
        data: result.driver,
        message: 'Driver created successfully as a managed fleet entity',
      });
    } catch (error) {
      next(error);
    }
  ...
  static async updateStatus(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const action =
        req.body.status === 'INACTIVE' || req.body.status === 'SUSPENDED'
          ? AuditAction.DRIVER_DEACTIVATED
