...
export class DashboardController {
  ...
  static async getFuelCost(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await DashboardService.getFuelCost(req.query as any) }); } catch (e) { next(e); }
  }
  static async getVehicleUsage(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await DashboardService.getVehicleUsage(req.query as any) }); } catch (e) { next(e); }
  }
  static async getDriverUsage(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await DashboardService.getDriverUsage(req.query as any) }); } catch (e) { next(e); }
  }
  static async getStationUsage(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await DashboardService.getStationUsage(req.query as any) }); } catch (e) { next(e); }
  }
  static async getRiskDistribution(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await DashboardService.getRiskDistribution(req.query as any) }); } catch (e) { next(e); }
  }
  static async getConsumptionTrend(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json({ success: true, data: await DashboardService.getConsumptionTrend(req.query as any) }); } catch (e) { next(e); }
  }
  static async getDriverDashboard(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const data = await DashboardService.getDriverSummary(req.user!.driverId!);
      res.json({ success: true, data });
    } catch (error) { next(error); }
  }
}
